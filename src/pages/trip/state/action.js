...
export const getAll = async (dispatch, token, status, curPage, resultPerPage, query) => {
  let url = `/api/admin/trip/?status=${status}&keyword=${query}&resultPerPage=${resultPerPage}&currentPage=${curPage}`;
...
export const getDetails = async (dispatch, token, id) => {
  ...
  try {
    ...
    const { data } = await axiosInstance.get(`/api/admin/trip/${id}`, {
      headers: { Authorization: token },
    });
